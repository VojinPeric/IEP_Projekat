...
def blockchain_decision(uuid, voters):
    ...
    if len(voters) == 0:
        return { "message": "Voters list is empty." }, 400
    if len(voters) % 2 == 0:
    ...
    approve_transaction = proposal_contract.functions.approve().build_transaction({ "from": admin_account, "gas": 300000 })
    reject_transaction = proposal_contract.functions.reject().build_transaction({ "from": admin_account, "gas": 300000 })
...
def normal_decision(uuid, approved):
    if uuid is None or not isinstance(uuid, str) or len(uuid) == 0:
        return { "message": "Field uuid is missing." }, 400
    if approved is None:
...
def decision():
    ...
    voters = body.get("voters", None)

    if approved is None:
        return blockchain_decision(uuid, voters)
    else:
        return normal_decision(uuid, approved)
